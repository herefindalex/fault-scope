...
目前是 **`0.0.x` 預覽版**，已發布七個案例：案例 01「應該再送一次嗎？」探討結果不明時的重試；案例 02「舊工作者還能提交嗎？」探討接手後的提交權限；案例 03「資料庫已提交，事件在哪裡？」探討資料庫提交與事件發布之間的持久性缺口；案例 04「消費端明明處理完了，為什麼又執行一次？」探討重新投遞與重複獎勵；案例 05「到底哪個事件比較新？」探討來源版本順序與投影狀態倒退；案例 06「讀取成功了，但資料夠新嗎？」探討寫入後確認所需的最低版本；案例 07「取消後，工作真的停了嗎？」探討結果交接卡住時的取消路徑。範例是用來教學的合成模型，不是實際雲端服務 SDK。
