...
目前是 **`0.0.x` 預覽版**，已發布三個案例：案例 01「應該再送一次嗎？」探討結果不明時的重試；案例 02「舊工作者還能提交嗎？」探討接手後的提交權限；案例 03「資料庫已提交，事件在哪裡？」探討資料庫提交與事件發布之間的持久性缺口。範例是用來教學的合成模型，不是實際雲端服務 SDK。
...
**人類語言（Human Locale）**改變介面與說明；**程式碼視角（Code Lens）**改變程式碼呈現，兩者互不影響。例如可以選擇繁體中文說明搭配 Go 程式碼。20 個人類語言的介面與案例 01–03 訊息均已補齊；其中 19 個非英語語言仍是**未經人工審校的 beta 翻譯**。程式碼視角有 Go、TypeScript、Python、Java、PHP、C、C++。
