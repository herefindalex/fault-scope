...
Human Locale is the first path segment: `/en/`, `/ja/cases/can-the-old-worker-still-commit/`, and `/ar/languages/c/`. All published Case slugs and all Code Lens slugs stay stable across locales. Case identity uses canonical IDs `fs-c01` through `fs-c08`, not translated URLs. `/{locale}/cases/` lists the published Cases. An unprefixed `/cases/{slug}/` remains a noindex language-selection entry page for each published Case.
