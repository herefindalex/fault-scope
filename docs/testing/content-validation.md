...
`go run ./tools check` regenerates Code Lens snippets, validates the eight published Cases and all public Human Locale catalogs, then runs the web and Go checks. `go run ./tools check --all` also compiles and executes the seven Code Lens fixture suites. See [developer commands](../development/commands.md).
...
- Every public locale (`source`, `beta`, or `reviewed`) has complete UI and Case 01–08 message sets and step coverage.
