...
The [locale registry](../../web/i18n/registry.json) holds each locale's ID, names, direction, and status. Shared UI strings live in `web/i18n/messages/<locale>.json`. Case strings live in `web/content/cases/<case-id>/locales/<locale>.json` for every published Case (`fs-c01` through `fs-c08`). The corresponding English files are the canonical sources for required message keys and placeholders.
...
If a development server is already running when you edit Case 02–08 copy, run `go run ./tools generate` to refresh its combined catalog map before reloading the page.
