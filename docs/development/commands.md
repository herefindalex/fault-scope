...
| `generate` | Extracts Case-scoped Code Lens regions and regenerates the combined Cases 02–08 locale map | Go and registered source/catalog files |
...
| `check --all` | Also runs all seven Code Lens compilers and behavioral fixture suites for Cases 01–08 | Full toolchain set |
...
`dev` and `preview` accept `--open`, `--locale <id>`, `--case <id-or-slug>`, `--language <lens>`, and `--mode guided|challenge|deep-dive`. The Case selector accepts all eight published IDs and slugs. These options choose the browser URL when `--open` is present. `--include-drafts` enables explicitly requested draft routes; `--listen` controls the executable's address.
