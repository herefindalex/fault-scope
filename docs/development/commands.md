...
| `check --all` | Also runs all seven Code Lens compilers and behavioral fixture suites for Cases 01–04 | Full toolchain set |
...
`dev` and `preview` accept `--open`, `--locale <id>`, `--case <id-or-slug>`, `--language <lens>`, and `--mode guided|challenge|deep-dive`. The Case selector accepts all four published IDs and slugs. These options choose the browser URL when `--open` is present. `--include-drafts` enables explicitly requested draft routes; `--listen` controls the executable's address.
